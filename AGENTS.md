...
同じ品質でLP・ホームページ・WordPress・管理画面・ポートフォリオ・SaaSを提案・設計・制作できる状態を維持する。

Design System（`knowledge/design-system/`）はすべてのプロダクトで共通利用できるデザイン基盤として構築・運用する。
...
### production-rules.md に従う5原則

1. **ナレッジベース優先**: 独自判断より `knowledge/` 内の既存知識を優先する
2. **Design System優先**: `knowledge/design-system/` のライブラリを必ず参照し、ゼロからデザインしない
3. **既存Template優先**: `templates-production/` の業種別テンプレートを最初に参照する
4. **ライブラリ組み合わせ制作**: Layout + Photo + Components + Motion + Spacing + Typography + Color を組み合わせて制作する
5. **Case Study保存必須**: 制作完了後は必ず `knowledge/case-studies/` へ記録する
...
業種判定
  ↓
Section Layout 選定（design-system/layouts/）
  ↓
Photo Composition 選定（design-system/photo/）
  ↓
UI Components 選定（design-system/components/）
  ↓
Motion 選定（design-system/motion/）
  ↓
Spacing 設計（design-system/spacing/）
  ↓
Typography 選定（design-system/typography/）
  ↓
Color System 決定（design-system/colors/）
  ↓
Visual Library / Asset Management
  ↓
制作
  ↓
Case Study 保存
...
Design System のライブラリが「準備中」の場合は既存の `knowledge/lp/` `knowledge/web/` で補完する。
...
**Design System（最優先）**
```
knowledge/design-system/layouts/hero-pattern-map.md       → Hero業種×CV目的マップ（★Hero選定FIRST）
knowledge/design-system/layouts/hero-layout-library.md    → Heroパターン詳細（HERO-XXX）55パターン
knowledge/design-system/layouts/hero-ranking.md           → HeroランキングCV/デザイン/モバイル
knowledge/design-system/layouts/feature-pattern-map.md    → Feature業種×CV目的マップ（★Feature選定FIRST）
knowledge/design-system/layouts/feature-layout-library.md → Featureパターン詳細（FEATURE-XXX）55パターン
knowledge/design-system/layouts/feature-ranking.md        → FeatureランキングCV/デザイン/スマホ/汎用性
knowledge/design-system/layouts/cta-layout-library.md     → CTAレイアウト
knowledge/design-system/layouts/voice-layout-library.md → Voiceレイアウト
knowledge/design-system/layouts/faq-layout-library.md   → FAQレイアウト
knowledge/design-system/layouts/flow-layout-library.md  → Flowレイアウト
knowledge/design-system/layouts/pricing-layout-library.md → Pricingレイアウト
knowledge/design-system/layouts/profile-layout-library.md → Profileレイアウト
knowledge/design-system/layouts/contact-layout-library.md → Contactレイアウト
knowledge/design-system/layouts/footer-layout-library.md → Footerレイアウト
knowledge/design-system/photo/photo-composition-library.md → 写真構図
knowledge/design-system/photo/visual-composition-rules.md  → ビジュアルルール
knowledge/design-system/components/ui-component-library.md → UIコンポーネント
knowledge/design-system/components/button-library.md    → ボタン
knowledge/design-system/components/card-library.md      → カード
knowledge/design-system/components/form-library.md      → フォーム
knowledge/design-system/components/badge-library.md     → バッジ
knowledge/design-system/motion/motion-library.md        → アニメーション
knowledge/design-system/spacing/spacing-library.md      → 余白
knowledge/design-system/typography/typography-library.md → フォント・スケール
knowledge/design-system/colors/color-system.md          → カラーシステム
```

**LP・業種知識**
```
knowledge/lp/lp-design-dictionary.md   → 構造パターン（Pattern-LP-XXX）
knowledge/lp/industry-patterns.md      → 業種別パターン（IND-XXX）
knowledge/lp/fv-library.md             → FVパターン（FV-XXX）
knowledge/lp/cta-library.md            → CTAパターン（CTA-XXX）
knowledge/lp/emotional-triggers.md     → 感情訴求トリガー（TRIG-XXX）
knowledge/lp/offer-library.md          → オファー設計（OFFER-XXX）
knowledge/assets/industry-asset-patterns.md → 業種別素材構成
knowledge/assets/asset-management-rules.md  → 素材管理ルール
knowledge/inspirations/                → 参考事例
```

**Design System 選定ルール（必須）**
- Design System のライブラリを参照し、各セクションのパターンを選択してから制作する
- **ライブラリを組み合わせて制作すること。ゼロからデザインを考えることは禁止**
- 「準備中」のライブラリは `knowledge/lp/` `knowledge/web/` で補完する

**Hero Design System 選定ルール**
1. `hero-pattern-map.md` → 業種×CV目的マップで候補を絞る
2. `hero-ranking.md` → CV期待値・モバイル適性を確認
3. `hero-layout-library.md` → HERO-XXX を選定して記録
- **ゼロからHeroを設計することを禁止する**

**Feature Design System 選定ルール**
1. `feature-pattern-map.md` → 業種×CV目的マップで候補を絞る
2. `feature-ranking.md` → CV期待値・スマホ適性・汎用性を確認
3. `feature-layout-library.md` → FEATURE-XXX を選定して記録
- **ゼロからFeatureを設計することを禁止する**
- Featureセクション制作時は必ず `feature-pattern-map.md` から始めること
