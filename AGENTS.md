...
knowledge/lp/fv-library.md                        → FVレイアウトパターン確認（FV-XXX）+ 選定フローチャート
knowledge/lp/fv-copy-library.md                   → FVコピーパターン確認（FV-COPY-XXX）★必須
...
knowledge/design/visual-library.md                → ビジュアルパターン確認（VISUAL-XXX）★必須
knowledge/design/fv-visual-library.md             → FVビジュアルレイアウト確認（FV-VISUAL-XXX）★必須
knowledge/design/industry-visual-patterns.md      → 業種別ビジュアル仕様確認（IND-V-XXX）★必須
knowledge/design/image-direction-rules.md         → 画像ディレクション・AIプロンプト確認
knowledge/assets/industry-asset-patterns.md      → 業種別必要素材一覧★必須
knowledge/assets/asset-checklist.md              → 素材チェックリスト確認
knowledge/assets/chatgpt-image-prompt-library.md → ChatGPT生成プロンプト確認
knowledge/assets/photo-source-library.md         → フォト素材・検索ワード確認
knowledge/assets/illustration-source-library.md  → イラスト素材確認
knowledge/assets/icon-source-library.md          → アイコン選定確認
knowledge/web/design-system.md                   → デザイントークン確認
knowledge/web/ui-patterns.md                     → UIコンポーネント確認
knowledge/inspirations/                          → 参考事例確認
```

**FVコピーについての重要ルール（AGENTS必読）**:
- FVコピーを新規作成する前に、必ず `fv-copy-library.md` を検索すること
- 既存の FV-COPY-XXX パターンの流用・カスタマイズを最優先とする
- 新規作成が必要な場合のみ末尾に追記し、Learning Reason を記録する
- FVコピー選定時は TRIG-XXX（感情）と OFFER-XXX（オファー）を同時に決定する

**Asset Management System 必読ルール（最重要）**:
- 画像選定・画像生成・FV設計を行う場合は必ず `knowledge/assets/` を参照すること
- **Claude Codeは画像生成を行わない** — 生成が必要な場合は ChatGPT（DALL·E 3）のみ使用
- Claude Codeの役割: 素材設計 / 検索ワード作成 / ChatGPT用プロンプト作成 / 配置指示
- LP制作前に `asset-management-rules.md` の「素材設計書」を必ず出力する
- 素材調達方法: CLIENT（最優先）→ フリー素材 → ChatGPT生成 の順で判断する
- ライセンス確認（asset-management-rules.md）を必ず実施してから素材を使用する

**ビジュアル設計についての重要ルール（AGENTS必読）**:
- 画像選定・生成・FV設計時は必ず `knowledge/design/` の4ファイルを参照すること
- 業種確定後は `industry-visual-patterns.md` の IND-V-XXX を必ず確認する
- FVビジュアルは `fv-visual-library.md` の「FV-VISUAL選定フローチャート」で選定する
- AI画像生成プロンプトは `image-direction-rules.md` の業種別プロンプトを流用する
- 各業種の「NGパターン」（industry-visual-patterns.md）を必ず確認・回避する
- ビジュアルの感情連携: `emotional-triggers.md` の TRIG-XXX とビジュアル仕様を整合させる
...
STEP4.3: FVビジュアル選定（fv-visual-library.md + industry-visual-patterns.md）★NEW
STEP4.4: 素材設計（assets/industry-asset-patterns.md → 素材設計書出力）★NEW
STEP4.5: FVコピー選定（fv-copy-library.md）★NEW
...
├── design/                         ← ビジュアルデザイン規則（Visual Library System）★NEW
│   ├── visual-library.md           ← ビジュアルパターン辞典（VISUAL-001〜044）
│   ├── fv-visual-library.md        ← FVビジュアルレイアウト（FV-VISUAL-001〜012）
│   ├── industry-visual-patterns.md ← 業種別ビジュアル仕様（IND-V-001〜006）
│   └── image-direction-rules.md    ← 撮影ディレクション・AI生成プロンプト（IND-IMG-001〜006）
├── assets/                         ← Asset Management System 2.0 ★NEW
│   ├── asset-management-rules.md   ← 素材管理統一規則（ChatGPT専用・役割分担）
│   ├── photo-source-library.md     ← フォト素材サイト・業種別検索ワード
│   ├── illustration-source-library.md ← イラスト素材サイト（unDraw/Storyset等）
│   ├── icon-source-library.md      ← アイコンライブラリ（Lucide標準採用）
│   ├── chatgpt-image-prompt-library.md ← ChatGPT画像生成プロンプト（IMAGE-PROMPT-001〜012）
│   ├── industry-asset-patterns.md  ← 業種別素材構成一覧（IND-ASSET-001〜006）
│   └── asset-checklist.md          ← LP制作時 素材確認チェックリスト
...
| バージョン | 更新日 | 変更内容 |
|----------|--------|---------|
| 2.1 | 2026-06-20 | 初版（ナレッジ運用ルール整備） |
| 2.2 | 2026-06-22 | 分析プロトコル強化 |
| 2.3 | 2026-06-23 | 制作フロー追加 |
| 2.4 | 2026-06-24 | emotional-triggers / offer-library を Step1ナレッジ検索リストに追加 |
| 2.5 | 2026-06-24 | FV Copy Library（fv-copy-library.md）を必須検索リストに追加。FVコピー新規作成前の既存パターン検索ルールを明記 |
| 2.6 | 2026-06-25 | Visual Library System（knowledge/design/ 全4ファイル）を必須検索リストに追加。ビジュアル設計ルール・STEP4.3追加 |
| 2.7 | 2026-06-25 | Asset Management System 2.0（knowledge/assets/ 全7ファイル）追加。ChatGPT専用・画像生成禁止ルール・STEP4.4追加 |
