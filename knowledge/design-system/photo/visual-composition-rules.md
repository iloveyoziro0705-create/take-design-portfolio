...
### 選定ルール：日本語フリーイラスト素材サイト（2026-07-12 追加・国内サイトを優先）

**2026年7月12日以降、日本人らしいイラスト・日本語の資料に馴染むイラストが必要な場合は、unDraw/Storyset（海外・抽象的な人物表現）より先に、以下の日本国内イラスト素材サイトを優先して探すこと。** 写真素材と同様、いずれも通常のブラウザUser-Agentを付けた`curl`で直接HTML取得可能（Bot対策不要）。**このうち`tyoudoii-illust.com`・`shigureni.com`・`loosedrawing.com`の3サイトは相互リンクされた「提携サイト」であり、同一運営ネットワーク内の姉妹サイトであることが確認できた**（各サイトのフッターに互いへのリンクあり）。

#### 1. ソコスト（soco-st.com） — シンプルな線画系、PNG/SVG/EPS対応
- 検索: カテゴリ`/category/{name}`（人物=`/category/person`）、タグ`/tag/{name}`（例: `/tag/woman`）
- 詳細ページ: `/{ID}`（例: `/10199`）。`og:image`に`wp-content/uploads/{年}/{月}/ogp_{ID}.png`のプレビューがあるが、実際のダウンロード（PNG/SVG/EPS選択）はJSボタン（`download-method="js"`）経由のため、curlだけで完結せずPlaywrightでのクリック操作が必要な場合がある
- ライセンス（`/guide`で確認済み）: 個人・法人・商用・非商用問わず無料、事前連絡・クレジット表記不要、**点数制限なし**。ただし画像への直リンクは禁止（必ずダウンロードして自サイトにアップロードすること）

#### 2. 素材Good（sozai-good.com） — 汎用イラスト、フレーム・背景も充実
- 検索: `/search?search_words={URLエンコード日本語}`
- 詳細ページ: `/illust/{カテゴリ}/{サブカテゴリ}/{ID}`
- ライセンス（`/rule`で確認済み）: 商用利用OK・クレジット不要・加工OK・個人利用OK。禁止事項は素材を利用した商標登録など

#### 3. ちょうどいいイラスト（tyoudoii-illust.com） — ビジネスシーン特化、8カテゴリ200+タグ
- 検索: カテゴリ`/category/{name}/`（人物=`/category/人物/`、URLエンコードは`%e4%ba%ba%e7%89%a9`）、通常のWordPress検索`/?s={query}`も動作確認済み
- 詳細ページ: `/{ID}/`（例: `/10467/`）。画像は`og:image`から`wp-content/uploads/{年}/{月}/{ファイル名}.png`を取得
- ライセンス（`/利用規約/`で確認済み・URL自体が日本語のためURLエンコードされる）: 商用可、クレジット表記・事前連絡不要。禁止事項はイラスト自体の販売・再配布、LINEスタンプ等「イラスト自体が商品になる」利用

#### 4. shigureni free illust（shigureni.com） — 素朴でかわいい女の子キャラクター系
- 検索: ジャンル`/genre/{name}`（例: `/genre/business`）、キーワード`/keyword/{word}`
- 詳細ページ: `/illust/{ID}`
- **⚠️ライセンスに重要な制限あり（`/terms`で確認済み）**: 商用・非商用・個人・法人問わず無料で使えるが、**商用利用で1つの制作物につき6点目以降を使う場合は1点あたり1,000円（税込）のライセンス購入が必要**（5点までは無料）。1つのLPで多数のイラストを使う設計をする場合はこの上限に注意し、5点を超えそうならshigureniへの依存を減らし他サイトと組み合わせること。禁止事項はLINEスタンプ化・素材のそのまま/加工での再配布販売・AI学習利用・商標登録など

#### 5. ランランド（web-sozai.com） — ビジネスシーンの人物イラストにIllustrator(AI)データ付き
- 検索: カテゴリ`/lists/list{番号}/`、タグ`/tax{番号}/{タグ名}/`
- 画像は`https://web-sozai.com/wp-content/uploads/{ハッシュ}.png`と`.ai`（Illustratorデータ）の両方が直リンクで存在し、PNG/AIどちらも登録不要でダウンロード可能
- ライセンス: 商用・非商用問わず利用可、再配布・販売禁止、著作権譲渡不可

#### 6. ガーリー素材（girlysozai.com） — ふんわりかわいい系（ゆるねこ・手書き風シリーズ）
- 検索: カテゴリ`/person/`, `/livingthing/`等
- 詳細ページ: `/person/{ID}/`
- ライセンス（`/about/`で確認済み）: 加工・商用利用いずれも問題なし、SNSアイコン利用も可、グッズ化して販売するのも可（素材そのまま/加工しての「素材としての」再配布・二次配布・虚偽の自作申告のみ禁止）。著作権は放棄されていないため商標登録は不可。クレジット表記の要否は規約に明記が見当たらず、必須ではないと考えられるが記載なし（要事前確認の余地あり）

#### 7. Loose Drawing（loosedrawing.com） — シンプルな手描き風、ビジネス用途向け落ち着いたタッチ
- 検索: カテゴリ13分類（ビジネス/感情/人間/教育/モノ/生活/動物/自然等）
- 詳細ページ: `/illust/{ID}/`。画像は`og:image`から`assets/media/illustrations/jpg/{ID}.jpg`を取得
- ライセンス（`/terms/`で確認済み）: 商用/非商用・個人/法人問わず無料、連絡不要、クレジット表記も必須ではない（任意でリンク紹介歓迎）。編集・加工OK。禁止事項はイラスト自体をコンテンツ・商品として再配布・販売する行為（LINEスタンプ等）。**tyoudoii-illust.com・shigureni.comの姉妹サイト**

#### 8. Tech Pic（tech-pic.com） — IT/テック系ビジネスイラスト特化（AI・アプリ・グラフ・近未来オフィス等）、SPA/API構成
- Nuxt/Vue製のSPAで、カテゴリ一覧・素材データはAPI経由で配信されている（`/materials/{カテゴリID}/{サブカテゴリID}`形式のURL）。curlで静的HTMLとして取得しづらい場合はPlaywrightでのレンダリング確認が必要になる可能性がある
- **⚠️無料版に利用制限あり（サイト内`ご利用規定`で確認済み）**: 無料は**低解像度800×800pxのPNGのみ**、**1つの資料・Webサイト・ブログにつき20点まで**、商用利用可、素材そのままの配布・販売は不可（加工は可）。有料版（開発中、記事執筆時点）でSVG・点数無制限に対応予定。著作権は運営会社「そのデザイン株式会社」が保有
- 強み: IT/SaaS/AI系LP（ai-school-lp、elearning-lp等の系統）にぴったりの近未来オフィス・AI活用シーンが専門的に揃っている

### 🚨 利用制限があるサイトの使用ルール（shigureni.com / tech-pic.com — 使用前に必ず参照）

shigureni.comとtech-pic.comは無料利用に**点数・解像度の上限**があり、規約違反や意図しない追加課金を避けるため、**この2サイトからイラストを1点でも使う際は、作業に入る前に必ずこのルールを読み、使用点数をその場で数えてから追加すること**。他6サイト（ソコスト/素材Good/ちょうどいいイラスト/ランランド/ガーリー素材/Loose Drawing）にはこの点数制限はない。

#### shigureni.com のルール
| 項目 | 内容 |
|------|------|
| 無料点数の上限 | **1つの制作物（＝1つのLP・Webサイト単位）につき5点まで** |
| 上限超過時 | 6点目以降は**1点につき1,000円（税込）の追加ライセンス購入が必須**（無断で使用してはならない） |
| 上限の数え方 | 「1つの制作物」＝納品する1つのLP/Webサイト全体を指す。トップページ・下層ページを合わせたサイト全体でカウントする（ページ単位ではない） |
| 許可される加工 | サイズ変更・色変更・文字入れ・トリミング・反転・簡単な合成・アニメーション |
| **禁止される加工** | 顔パーツの追加、性別の変更など「違う人物に見える」改変／当サイトのイラストをベースに別イラストを作る行為／サイトのイメージを損なう過度な加工 |
| 禁止用途 | LINEスタンプ・デザインテンプレート・Tシャツ・シール・絵本等「イラスト自体が商品」になる利用、そのまま/加工しての再配布・販売、商標登録、公序良俗違反、性的コンテンツ、選挙・政党・宗教関連、差別・誹謗中傷、**AI学習への利用** |
| クレジット表記 | 不要 |

**運用手順**: shigureniのイラストを1点使う前に、そのLP（サイト全体）で既に何点使用済みかを数える → **5点に達した、または5点目を使おうとしている場合は、6点目以降を追加する前に必ずユーザーに「6点目以降は1点1,000円の追加ライセンスが必要になりますが、購入して使用しますか？それとも他サイトの素材に切り替えますか？」と確認する**。ユーザーの承認なしに有料ライセンスを前提とした使用点数まで増やさないこと。5点以内に収まる設計にするか、girlysozai/loosedrawing等の同系統サイトと組み合わせて分散させるのが基本方針。

#### tech-pic.com のルール
| 項目 | 内容 |
|------|------|
| 無料点数の上限 | **1つの資料・Webサイト・ブログにつき20点まで**（教育機関の授業資料等は例外で無制限だが、通常のLP制作には該当しない） |
| 解像度の上限 | 無料版は**縦800px×横800pxのPNGデータのみ**（SVG・高解像度は有料版待ち、記事執筆時点で未提供） |
| 許可用途 | 書籍、チラシ、パンフレット、Web、パワーポイント、動画のプレゼン資料など。商用利用可 |
| 禁止事項 | 素材そのままの配布・販売（資料や商品の一部として使うのは可） |
| 加工 | 可 |
| 著作権 | 運営会社「そのデザイン株式会社」が保有 |

**運用手順**: tech-picのイラストを使う前に、そのLP（サイト全体）で既に何点使用済みかを数える → **20点に達している場合は追加使用を止め、他サイト（Loose Drawing/ちょうどいいイラスト等のビジネス系）に切り替える**。800×800pxという解像度制約があるため、Hero等の大型ビジュアルには不向き（アイコン的な用途・カード内の小〜中サイズ挿絵向き）と判断し、大きく引き伸ばして使わないこと。

#### 参考: まとめサイト（webclips.jp/design/illustration-free/）
上記8サイト以外の候補が必要な場合、`https://webclips.jp/design/illustration-free/` は国内38サイト・海外16サイトのフリーイラスト素材サイトをまとめた記事で、スタイル別（線画・アイソメトリック・フラット・ビジネス等）に探せる。個別のダウンロード・検索フローはリンク先サイトごとに都度確認が必要（このサイト自体は素材の配布元ではなくリンク集）。

#### 調達フローの優先順位（2026-07-12以降の標準運用、イラスト版）
1. 求める絵柄（ビジネス系ならちょうどいいイラスト/ランランド/Tech Pic、かわいい系ならガーリー素材/shigureni、シンプル線画ならソコスト/Loose Drawing）に応じて上記8サイトから日本語キーワードで検索する
2. **shigureni.com・tech-pic.comを使う場合は、必ず上記「🚨利用制限があるサイトの使用ルール」を参照し、使用点数をカウントしてから追加する**（shigureniは1LP5点まで無料・6点目以降は1点1,000円、tech-picは1LP20点まで・解像度800×800pxのみ）。上限に達する見込みなら、他6サイトに分散させるか設計段階で点数を調整する
3. 8サイトで見つからない場合のみ、従来のunDraw（帰属不要・無表情）/Storyset（要帰属表示・表情豊か）にフォールバックする
