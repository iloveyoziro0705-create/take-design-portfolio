...
## DESINS-012: 重厚ブラック×ホワイト×アクセントレッド（建設・製造採用LP系）

**出典**: 戸田建設 採用LP / 2026-06-22-toda-construction-recruit-lp.md  
**業種タグ**: #construction #manufacturing #recruit #corporate  
**LP-Rank**: C+（56/100）  
**参考箇所**: カラーパレット全体設計（デザイン統一スコア10/10実証済み）

### 参考要素
- 背景: `#0d0d0d`（ほぼ黒）— 建設・ゼネコン・製造業の「重厚感・力強さ」を表現
- テキスト: `#ffffff`（純白）— 暗背景上での最大可読性確保
- アクセント: `#cc0000`（力強いレッド）— 警戒色でなく「情熱・行動・決断」の色相として採用訴求に転用
- GEN-002中でデザイン統一スコア唯一の10/10を獲得（他項目は低スコアながらデザイン面は圧倒的）
- 「TEAM TODA」等の採用ブランド名との親和性が高く、チーム参加の力強さを視覚的に表現

### 適用可能業種
建設・ゼネコン・警備・製造・インフラ・物流・重工業の採用LP

### 実装メモ
```css
:root {
  --bg:      #0d0d0d;  /* ほぼ黒（重厚感）*/
  --surface: #1a1a1a;  /* ダークグレー（カード背景）*/
  --text:    #ffffff;  /* 純白 */
  --sub:     #cccccc;  /* ライトグレー（サブテキスト）*/
  --accent:  #cc0000;  /* 力強いレッド */
  --accent-hover: #ee0000;
  --border:  #333333;
}
/* CTAボタン */
...
/* セクション区切り */
.section-divider { border-top: 2px solid var(--accent); }
```

---

## DESINS-013: 格調パープル×ホワイト×ダークブラック（高級和食採用LP系）

**出典**: 割烹 吉帆楼 採用LP / 2026-06-22-yoshihoryu-restaurant-recruit-lp.md  
**業種タグ**: #japanese-dining #restaurant #recruit #luxury #traditional  
**LP-Rank**: C+（53/100）  
**参考箇所**: カラーパレット全体設計

### 参考要素
- 背景: `#7b4f8a`〜`#6b3fa0`（格調あるパープル）— 和食の「格」「伝統」「品格」を色相で表現
- テキスト: `#ffffff`（純白）— 暗背景での可読性確保
- コンテンツ背景: `#ffffff`（純白）— パープルとのコントラストで本文エリアを明確に区分
- アクセント: `#1a1a1a`（ほぼ黒）— パープルとのコントラストで引き締め効果
- 採用LPで「接客・おもてなし・日本料理」の文化的価値を色で訴求するパレット
- 「スキル獲得型FVコピー（CV向上ポイント#26）」との相性が特に高い

### 適用可能業種
高級和食・割烹・料亭・旅館・茶道・伝統芸能・おもてなし業全般の採用LP

### 実装メモ
```css
:root {
  --bg:           #7b4f8a;  /* 格調パープル（FV背景）*/
  --bg-deep:      #4a2060;  /* ディープパープル（フッター等）*/
  --surface:      #ffffff;  /* 白（コンテンツ背景）*/
  --text:         #1a1a1a;  /* ほぼ黒（本文）*/
  --text-on-purple: #ffffff; /* パープル背景上の白文字 */
  --sub:          #5a3a6e;  /* ミッドパープル（装飾線）*/
  --accent:       #f0e0ff;  /* ペールラベンダー（装飾・ハイライト）*/
}
/* FV: パープルグラデーション背景 */
.hero {
  background: linear-gradient(135deg, #7b4f8a 0%, #4a2060 100%);
  color: #fff;
}
```
...
## DESINS-014: ホワイト × ゴールド × ピンク（プレミアム美容歯科パレット）

**出典**: にしむら歯科クリニック（GEN-004-003）/ 2026-06-22-nishimura-dental-lp.md  
**業種タグ**: #dental #cosmetic-dentistry #beauty-clinic #luxury #feminine  
**LP-Rank**: D（配色参考のみ）  
**参考箇所**: カラーパレット全体設計

### 参考要素
- ベース: クリーンホワイト（`#ffffff`〜`#f5f5f5`）→ 清潔感・医療的信頼感の基盤
- アクセント1: ソフトゴールド（`#d4af85`〜`#c8a56a`系）→ 高級感・プレミアム感の演出
- アクセント2: ライトピンク / ローズピンク（`#f5c5c5`〜`#e8a0a0`系）→ 女性向けの温かみと可愛らしさ
- 組み合わせ効果: 三色で「医療的清潔感（白）+ 高級感（ゴールド）+ 女性的温かみ（ピンク）」を同時表現
- 差別化: 一般的な「ブルー×ホワイト系」歯科サイトから視覚的に差別化（フェミニンな美容歯科ポジションを視覚的に宣言）

### 適用可能業種
美容歯科（ホワイトニング・審美歯科・矯正）・美容クリニック・エステ・ブライダルサロン・高級スキンケアEC

### 実装メモ
```css
:root {
  --bg:        #ffffff;          /* クリーンホワイト */
  --surface:   #f8f5f5;          /* ローズホワイト面 */
  --text:      #2d2020;          /* ウォームダークブラウン */
  --sub:       #7a6a6a;          /* ミッドウォームグレー */
  --gold:      #d4af85;          /* ソフトゴールド */
  --gold-dark: #c8a56a;          /* ディープゴールド */
  --pink:      #f5c5c5;          /* ライトローズ */
  --pink-dark: #e8a0a0;          /* ミッドローズ */
}
```

---

## DESINS-015: クリームホワイト × テラコッタ × ダークブラウン（ラグジュアリーナチュラル美容医療パレット）

**出典**: APOLLON CLINIC（GEN-004-008）/ 2026-06-22-apollon-clinic-cosmetic-lp.md  
**業種タグ**: #cosmetic-surgery #beauty-clinic #luxury #natural #warm  
**LP-Rank**: D（配色参考のみ）  
**参考箇所**: カラーパレット全体設計・アーチ型モチーフとの組み合わせ

### 参考要素
- ベース: クリームホワイト / オフホワイト（`#F5F0EA`〜`#FAF6F1`系）→ 純白より温かみがあり高級感を維持
- アクセント: テラコッタ / ローズブラウン（`#C4856A`〜`#B5745A`系）→ 温かみ・柔らかさ・フェミニン
- テキスト: ダークウォームブラウン（`#3D2B1F`〜`#2A1B0F`系）→ 純黒の冷たさを排除し全体の色温度を統一
- 組み合わせ効果: 3色全てが「温かみ」方向に統一され、「冷たい高級感（白×シルバー系）」ではなく「温かい高級感」を表現
- DESINS-001との差: DESINS-001はゴールドアクセント（華やかさ）、DESINS-015はテラコッタアクセント（柔らかさ・有機的）

### 適用可能業種
美容クリニック・美容外科・フェイシャルエステ・スキンケア・ウェルネスサロン・ヴィラ系宿泊・高級オーガニック

### 実装メモ
```css
:root {
  --bg:        #F5F0EA;          /* クリームオフホワイト */
  --surface:   #FAFAFA;          /* 白（カード背景）*/
  --text:      #3D2B1F;          /* ダークウォームブラウン */
  --sub:       #7A6050;          /* ミッドウォームブラウン */
  --accent:    #C4856A;          /* テラコッタ/ローズブラウン（CTAボタン等）*/
  --accent-lt: #E8C4B0;          /* ライトテラコッタ（ホバー・薄背景）*/
  --gold:      #D4B896;          /* ウォームゴールド（装飾ライン）*/
}
/* アーチ型モチーフと相性が良い（ロゴ・FV背景・写真クロップ等で統一）*/
```
...
- 登録事例数: 15（GEN-004 医療系LP辞典統合 2026-06-22 時点）
- 最終更新: 2026-06-22（DESINS-014〜015 追加 / 医療系LP辞典統合）
