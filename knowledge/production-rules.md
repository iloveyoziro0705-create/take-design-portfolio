...
3.  knowledge/lp/fv-library.md                       ← FVレイアウトパターン（FV-XXX）+ 選定フローチャート
4.  knowledge/lp/fv-copy-library.md                  ← FVコピーパターン（FV-COPY-XXX）★必須
5.  knowledge/lp/emotional-triggers.md               ← 感情訴求トリガー（TRIG-XXX）
6.  knowledge/lp/offer-library.md                    ← オファー設計パターン（OFFER-XXX）
7.  knowledge/design/visual-library.md               ← ビジュアルパターン（VISUAL-XXX）★必須
8.  knowledge/design/fv-visual-library.md            ← FVビジュアルレイアウト（FV-VISUAL-XXX）★必須
9.  knowledge/design/industry-visual-patterns.md     ← 業種別ビジュアル仕様（IND-V-XXX）★必須
10. knowledge/design/image-direction-rules.md        ← 画像ディレクション・プロンプト
11. knowledge/assets/industry-asset-patterns.md     ← 業種別素材構成（★必須）
12. knowledge/assets/asset-checklist.md             ← 素材チェックリスト
13. knowledge/assets/chatgpt-image-prompt-library.md ← ChatGPT生成プロンプト
14. knowledge/lp/cta-library.md                      ← CTAパターン（CTA-XXX）
15. knowledge/web/design-system.md                  ← デザイントークン・コンポーネント
16. knowledge/case-studies/                         ← 同業種の過去案件
```
...
| **ゼロから考えない** | 業種判定 → Template選択 → FV選択 → FVコピー選択 → 感情設計 → オファー設計 → CTA選択 → 制作 の順で進める |
...
### STEP 4.3: FVビジュアル選定 ★NEW

`knowledge/design/fv-visual-library.md` と `knowledge/design/industry-visual-patterns.md` を参照し、FVビジュアル仕様を確定する。

```
確認手順:
1. industry-visual-patterns.md で業種（IND-V-XXX）を確認
   → ターゲット心理×ビジュアルメッセージ / 推奨写真仕様 / カラーパレット確認
2. fv-visual-library.md の「FV-VISUAL選定フローチャート」でFV-VISUAL-XXXを選定
3. visual-library.md で使用するビジュアル要素（VISUAL-XXX）を選定
4. image-direction-rules.md で画像仕様・AIプロンプトを確認
5. 感情連携: emotional-triggers.md のTRIG-XXXとビジュアル整合性を確認
6. 選定結果を STEP 9 の LP設計書に記録する
```

**ビジュアル選定の優先順位**:
1. `industry-visual-patterns.md` の「3高CVパターン」から選ぶ
2. `fv-visual-library.md` の業種別推奨FV-VISUALを適用
3. 写真素材がない場合 → FV-VISUAL-011（グラデーション背景）またはFV-VISUAL-012（テキスト主体）を使用

**画像生成・選定時の禁止事項**:
- NGパターンに該当するビジュアル（industry-visual-patterns.md の各業種NGパターン参照）
- 医療: 誇大表現・ビフォーアフター（薬機法・景表法違反リスク）
- 美容: 過度な美肌レタッチ・効果誇大表示

---

### STEP 4.4: 素材設計（Asset Management）★NEW

`knowledge/assets/` のファイルを参照し、制作に必要な全素材を洗い出す。

```
確認手順:
1. industry-asset-patterns.md で業種の「必要素材一覧」を確認
2. asset-checklist.md で業種別チェックリストを作成
3. 各素材の調達方法を決定:
   - CLIENT（クライアント提供）→ 提供依頼リストを作成
   - フリー素材 → photo/illustration-source-library.md で検索ワードを確認
   - ChatGPT生成 → chatgpt-image-prompt-library.md でプロンプト確認
4. 「素材設計書」をLPに合わせて出力する（asset-management-rules.md 参照）
```

**重要ルール（必読）:**
- Claude Codeは画像生成を行わない
- 画像生成が必要な場合は ChatGPT（DALL·E 3）のみを使用する
- Claude Codeの役割: 素材設計 / 検索ワード作成 / プロンプト作成 / 配置指示

**素材設計書 出力内容（コーディング前に必ず出力）:**
1. 必要素材一覧（素材名 / 配置場所 / 優先度 / 調達方法）
2. クライアントへの提供依頼リスト
3. フリー素材検索ワード
4. ChatGPT画像生成プロンプト（IMAGE-PROMPT-XXX から流用）
5. 配置指示

---

### STEP 4.5: FVコピー選定 ★NEW

`knowledge/lp/fv-copy-library.md` を参照し、FV-COPY-XXX を選定する。

```
確認手順:
1. fv-copy-library.md の「パターン早引き一覧」で業種フィルタ
2. 「FV-COPY選定マトリクス」でターゲット状態を確認
3. 1〜3パターンを候補として選定
4. 感情トリガー（TRIG-XXX）を確認 → emotional-triggers.md で詳細確認
5. オファー（OFFER-XXX）を確認 → offer-library.md で詳細確認
6. 選定理由を STEP 9 の LP設計書に記録する
```

**FVコピー選定の優先順位**:
1. 既存パターンの「実例コピー」をそのままカスタマイズする
2. 複数パターンを組み合わせる（例: FV-COPY-001の悩み + FV-COPY-007のオファー）
3. 上記で対応できない場合のみ新規コピーを作成し、末尾に追記する

**コピータイプの選択基準**:
- ターゲットが悩みを自覚している → 悩み訴求型（001,011,021,041,051）
- 競合比較フェーズ → 比較訴求型（003,013,023,043,053）
- 先延ばし傾向あり → 損失回避型（004,014,024,044,054）
- 信頼性が課題 → 実績/権威訴求型（005,006,015,016,025,026）
...
業種判定
    ↓
Template選択（templates-production/）
    ↓
FVレイアウト選択（fv-library.md）
    ↓
FVビジュアル選択（fv-visual-library.md + industry-visual-patterns.md）
    ↓
素材設計・Asset Management（assets/industry-asset-patterns.md）★NEW
    ↓
FVコピー選択（fv-copy-library.md）
    ↓
感情設計（emotional-triggers.md）
    ↓
オファー設計（offer-library.md）
    ↓
CTA選択（cta-library.md）
    ↓
...
| 2026-06-24 | v1.1: FV Copy Library（STEP 4.5）追加。選定フローにFVコピー・感情・オファーの3ステップを明示。 |
| 2026-06-25 | v1.2: Visual Library System（STEP 4.3）追加。FVビジュアル選定・画像ディレクション・業種別ビジュアル仕様をフローに統合。 |
| 2026-06-25 | v1.3: Asset Management System 2.0（STEP 4.4）追加。Photo/Illustration/Icon/ChatGPT画像の統合管理フローを制作標準化。 |
