...
**作成日**: 2026-06-24
**更新日**: 2026-06-28
**ステータス**: Design System v1.0 フォルダ構成統合済み
...
```
【STEP 0: ガバナンス確認】
  knowledge/governance.md                                         ← SST マップ確認

【STEP 1: 業種パターン確認】
  knowledge/lp/industry-patterns.md                               ← 業種判定（IND-XXX）
  knowledge/lp/emotional-triggers.md                              ← 感情訴求トリガー（TRIG-XXX）
  knowledge/lp/offer-library.md                                   ← オファー設計（OFFER-XXX）

【STEP 2: Section Layout 選定（★Design System優先）】
  knowledge/design-system/layouts/hero-pattern-map.md             ← Hero業種×CV目的マップ
  knowledge/design-system/layouts/hero-layout-library.md         ← Heroパターン詳細（HERO-XXX）
  knowledge/design-system/layouts/hero-ranking.md                ← Heroランキング
  knowledge/design-system/layouts/cta-layout-library.md          ← CTAレイアウト（準備中）
  knowledge/design-system/layouts/feature-pattern-map.md          ← Feature業種×CV目的マップ
  knowledge/design-system/layouts/feature-layout-library.md      ← Featureレイアウト（FEATURE-XXX）55パターン
  knowledge/design-system/layouts/feature-ranking.md             ← FeatureランキングCV/デザイン/スマホ/汎用性
  knowledge/design-system/layouts/voice-layout-library.md        ← Voiceレイアウト（準備中）
  knowledge/design-system/layouts/faq-layout-library.md          ← FAQレイアウト（準備中）
  knowledge/design-system/layouts/flow-layout-library.md         ← Flowレイアウト（準備中）
  knowledge/design-system/layouts/pricing-layout-library.md      ← Pricingレイアウト（準備中）
  knowledge/design-system/layouts/profile-layout-library.md      ← Profileレイアウト（準備中）
  knowledge/design-system/layouts/contact-layout-library.md      ← Contactレイアウト（準備中）
  knowledge/design-system/layouts/footer-layout-library.md       ← Footerレイアウト（準備中）

【STEP 3: Photo Composition 選定】
  knowledge/design-system/photo/photo-composition-library.md     ← 写真構図パターン（準備中）
  knowledge/design-system/photo/visual-composition-rules.md      ← ビジュアル選定ルール（準備中）
  knowledge/assets/industry-asset-patterns.md                    ← 業種別素材構成
  knowledge/assets/asset-management-rules.md                     ← 素材管理ルール

【STEP 4: UI Components 選定】
  knowledge/design-system/components/ui-component-library.md     ← UIコンポーネント統合（準備中）
  knowledge/design-system/components/button-library.md           ← ボタン（準備中）
  knowledge/design-system/components/card-library.md             ← カード（準備中）
  knowledge/design-system/components/form-library.md             ← フォーム（準備中）
  knowledge/design-system/components/badge-library.md            ← バッジ（準備中）
  knowledge/lp/cta-library.md                                    ← CTAパターン（CTA-XXX）

【STEP 5: Motion 選定】
  knowledge/design-system/motion/motion-library.md               ← アニメーション（準備中）

【STEP 6: Spacing 設計】
  knowledge/design-system/spacing/spacing-library.md             ← 余白基準（準備中）

【STEP 7: Typography 選定】
  knowledge/design-system/typography/typography-library.md       ← フォント・スケール（準備中）

【STEP 8: Color System 決定】
  knowledge/design-system/colors/color-system.md                 ← カラーシステム（準備中）

【STEP 9: Visual Library / FV】
  knowledge/lp/fv-library.md                                     ← FVパターン（FV-XXX）

【STEP 10: Asset Management】
  knowledge/assets/                                              ← 素材一式

【STEP 11: Case Study確認】
  knowledge/case-studies/                                        ← 同業種の過去案件
```

> **注意**: 「準備中」のライブラリは今後構築予定。現時点では参照先が整備されたものを優先し、未整備の項目は既存ナレッジ（`knowledge/lp/` `knowledge/web/`）で補完する。
...
| **ゼロから考えない** | 業種判定 → Layout選択 → Photo → Components → Motion → Spacing → Typography → Color → 制作 の順で進める |
...
### STEP 3.5: Section Layout 選定（Design System優先）

`knowledge/design-system/layouts/` 配下のライブラリから各セクションのパターンを選定する。

```
選定手順:
1. hero-pattern-map.md の「業種×CV目的マップ」でHeroパターン候補を絞る
2. hero-ranking.md でCV期待値・モバイル適性を確認
3. hero-layout-library.md で詳細確認・選定（HERO-XXX を決定）
4. feature-pattern-map.md の「業種×CV目的マップ」でFeatureパターン候補を絞る
5. feature-ranking.md でCV期待値・スマホ適性・汎用性を確認
6. feature-layout-library.md で詳細確認・選定（FEATURE-XXX を決定）
7. 他セクション（Voice/FAQ/Flow/CTA等）も対応ライブラリで選定
8. 各選定パターンを STEP 9 の LP設計書に記録する
```

> **Featureセクション制作の絶対ルール**: ゼロからFeatureを設計することを禁止する。必ず `feature-pattern-map.md` から始め、`feature-layout-library.md` のパターンを選定してから制作すること。
...
**各セクションをゼロから設計することを禁止する。必ずライブラリからパターンを選定してから制作する。**
